...
class Chauffeur
{
    ...
    {
        $this->iCount = $this->getChaufferusCount();
    }

    function getChauffeursTable(){
        $list = $this->getChauffeursList();
        $array = array();
    ...
    function getChaufferusCount(){
        $s = "SELECT COUNT(*) FROM Gebruikers WHERE `Soort Gebruiker` = 'Chauffeur';";
        $db = new DataBase();
        $db->setQuery($s);
        $db->executeQuery();
        $r = $db->getResult();
